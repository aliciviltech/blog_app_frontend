import { ChevronDown, CrossIcon, MenuIcon, Moon, Search, Sun } from 'lucide-react'
  ...
  return (
    ...
    <div className={`Header  select-none relative ${searchBar && 'mb-[80px]'}  lg:mb-[0] px-2 flex lg:gap-10 justify-between items-center py-8 max-w-[1300px] mx-auto transition-all`}>
      ...
      <div className="leftSide lg:w-[50%] flex gap-6 items-center">
        ...
        {/* menubars */}
        <div ref={menuBarsRef} className="menuBars lg:hidden dark:text-white" onClick={handleShowMenu}>
          <MenuIcon />
        </div>

        {/* logo */}
        <div className="logo dark:text-white text-[22px] font-bold sm:block cursor-pointer" onClick={() => navigate('/')}>
          <span className='text-[var(--secondaryColor)]'>Baitul</span>Blog
        </div>

        {/* navigation bar */}
        <div ref={drawerRef} className={
          `navbar ${showMenu ? 'left-0 opacity-100' : 'left-[-100%] opacity-0 '} flex flex-col transition-all duration-700  absolute top-[70px] bg-black dark:bg-primaryDarkBg text-white  font-poppins shadow-lg w-[85%] min-h-[calc(100vh-90px)] z-20 py-10 ml-auto
          sm:w-[300px]
          lg:gap-4
          lg:static lg:opacity-100 lg:w-fit lg:min-h-fit lg:p-0 lg:bg-transparent lg:text-black lg:dark:text-white lg:shadow-none  lg:flex-row  `}>

          <div className='categories relative flex flex-col lg:items-center border-b border-white px-4 py-2 lg:border-none lg:p-0'>
            <div className='flex items-center cursor-pointer hover:text-primaryColor transition-all' onClick={() => showDrawersF('categories')}>Categories <ChevronDown className='' size={20} /></div>
            <CategoriesDrawer closeAllDrawers={closeAllDrawers} className={showDrawer.categories ? 'h-48' : 'h-0'} />
          </div>

          <div className='pages relative flex flex-col  lg:items-center border-b border-white px-4 py-2 lg:border-none lg:p-0'>
            <div className='flex items-center cursor-pointer hover:text-primaryColor transition-all' onClick={() => showDrawersF('pages')}>Pages<ChevronDown size={20} /></div>
            <PagesDrawer className={showDrawer.pages ? 'h-48' : 'h-0'}/>
          </div>

          <Link to={''} className='border-b border-white px-4 py-2 lg:border-none lg:p-0 hover:text-primaryColor transition-all'>Contact Us</Link>
          <Link to={''} className='border-b border-white px-4 py-2 lg:border-none lg:p-0 hover:text-primaryColor transition-all'>About Us</Link>
        </div>
      </div>

      {/* ======================== right side =============================== */}
      <div className="rightSide sm:grow-[0]  flex gap-6 items-center justify-between">
        ...
        {/* toggel theme mode */}
        <div class="p-2 rounded-lg bg-gray-200 cursor-pointer " onClick={handleTheme}>
          {
            currentMode === 'light' ?
              <Moon  size={16} />
              :
        ...
        {/* user info */}
        {
          activeUserRedux?.name ?
            <div ref={userDrawerRef} className="user relative sm:flex gap-2 items-center cursor-pointer" onClick={handleUserDrawer} >
              ...
              <p className='H5 hidden sm:block dark:text-white'>{activeUserRedux.name}</p>
              <ChevronDown className=' hidden sm:block dark:text-white' />
